...
- [~] 已为 Chaos Web 交互控件加入深/浅主题键盘焦点轮廓，工作区切换/归档是独立语义按钮；真实 desktop/mobile Playwright 覆盖 Tab/Enter 与工作区流程。axe-core 浏览器扫描显式启用 WCAG 2.0/2.1 A/AA、WCAG 2.2 AA 与 best-practice tag，检查空态及真实 WebSocket 消息/工作区状态，发现并修复 document title/lang 和语义主标题缺失；两态现无 serious/critical violations。完整中英文本地化缺键、长文本、屏幕阅读器实机及 macOS/Windows/Tauri 无障碍验收仍开放。（2026-09-25；`apps/chaos-ui/e2e/workspace-flow.pw.ts`、`apps/chaos-ui/index.html`、`src/style.css`）
