...
- [ ] 实现命令、写文件、网络和 MCP 工具的审批卡片，以及通用提问对话框。
- [ ] 审批记录绑定 session、tool call、参数摘要和不可伪造的 request ID。
...
- [ ] 复用现有 workspace RPC 的 list/read/range/get/put/search 能力；能力矩阵中已有项禁止重复实现。
- [ ] 实现增量文件树、模糊搜索、内容搜索和 `@` 文件候选。
- [ ] 附件支持类型/大小白名单、分块或流式传输、取消、进度、清理和配额。
- [ ] 所有路径执行 canonicalization、workspace confinement 和 symlink escape 测试。
...
- [ ] MCP：列表、连接状态、工具发现、OAuth/凭据、启停、重连和错误诊断。
- [ ] 插件：来源校验、安装、启用、配置、卸载和内置插件恢复。
- [ ] 技能：扫描、冲突提示、引用目录和同步策略。
...
- [ ] 工作流/子代理复用 engine 的取消、事件序号、恢复和审计机制。
