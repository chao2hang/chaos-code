...
- [x] CI `36178108811` passed separate Linux GUI browser, GUI Rust/frontend/protocol jobs and main Rust `fmt`, all-target `check`, strict workspace `clippy`, and full workspace tests with `RUST_MIN_STACK=16 MiB`; Rust job elapsed 33m07s (<60m). This stack setting fixes the reproducible `xai-grok-shell` current-thread actor test overflow found in run `36165469964`; package library tests (6,804 passed) and complete workspace tests pass locally at the same stack size. Gate executes ignored inventory and bounded brand drift checks. Remote status JSON: scratch `github-ci-stack16.json`。（2026-09-25）
