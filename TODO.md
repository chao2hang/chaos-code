...
- [x] CI `36133400667` passed separate Linux GUI browser, GUI Rust/frontend/protocol jobs and main Rust `fmt`, all-target `check`, strict workspace `clippy`, and full workspace `test`; Rust job elapsed 42m33s (<60m). Gate executes ignored inventory and bounded brand drift checks. Prior full suite `36094218127` also passed; latest local approval-outcome E2E is in private goal scratch `final-verification-round3.log`, and final remote status JSON in `github-ci-approval.json`。（2026-09-25）
...
- [x] Repository Playwright flows drive actual Engine/WebSocket/Vite desktop and narrow projects: workspace create/submit/switch/reload/archive, layout/theme, health/handshake, empty/cancel, demo approval reject/allow-to-adapter-failure and question answer, GFM/inline+block formatting, injected HTML inertness, denied relative/javascript hrefs and hardened external hrefs. Linux browser CI passed runs `36094218127` and `36133400667`。（2026-09-25）
