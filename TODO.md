...
- [~] engine/WebSocket 已提供受 root confinement 保护的目录列表、文本读取、搜索（最多 100 个匹配、单文件 1 MiB）；真实 WebSocket flow 已验证列表排序、空目录返回空 entries、`.chaos-staging` 与其内容不对外列出、目录读取以 `read_failed` 拒绝，且越界读取失败（`crates/codegen/xai-grok-web/tests/workspace_flow.rs`）。尚无文件树/搜索 UI，因为 files_listed entries 仅是名称字符串，缺乏不随文件名后缀猜测的 node kind、列表/读取 loading/error/empty 状态和 directory traversal/breadcrumb contract。误加原型 UI 会把 demo root/状态冒充完整 M2 行为；确定显示交互契约并补真实 Browser E2E 后再实现。模糊搜索和 `@` 文件候选仍待定。
