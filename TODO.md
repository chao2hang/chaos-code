...
- [~] engine/WebSocket 已提供受 root confinement 保护的目录列表、文本读取、搜索（最多 100 个匹配、单文件 1 MiB）；尚无文件树/搜索 UI，因为 files_listed entries 仅是名称字符串，缺乏不随文件名后缀猜测的 node kind、列表/读取 loading/error/empty 状态和 directory traversal/breadcrumb contract。误加原型 UI 会把 demo root/状态冒充完整 M2 行为；确定显示交互契约并补真实 Browser E2E 后再实现。模糊搜索和 `@` 文件候选仍待定。
...
- [~] 已为 Chaos Web 交互控件加入深/浅主题键盘焦点轮廓，工作区切换/归档是独立语义按钮；真实 desktop/mobile Playwright 覆盖 Tab/Enter 与工作区流程。曾尝试在 Playwright 加 axe-core 全页 WCAG 扫描，实际发现静态 HTML 缺 document title/lang/main heading。修复它们会改变所有静态输出，未获准的产品语言/屏幕阅读器合同不该机械假定；已撤下这个实验，不声称目前达成 M3 accessibility gate。此前已交付的 theme-aware focus-visible、semantic workspace controls 与 keyboard desktop/mobile tests 保留。完整中英文本地化缺键、长文本、屏幕阅读器实机及 macOS/Windows/Tauri 无障碍验收仍开放。（2026-09-25；`apps/chaos-ui/e2e/workspace-flow.pw.ts`、`apps/chaos-ui/index.html`、`src/style.css`）
