...
pub enum RetryDecision {
    ...
    /// Retry after stripping the unsupported `reasoning_effort` parameter
    /// (400 from a model/provider that rejects it). The caller checks
    /// whether effort was actually present; if not, upgrades to `Fatal`.
...
pub fn format_sampling_error(err: &SamplingError, retry_count: Option<u32>) -> String {
    ...
    match err {
        ...
        SamplingError::MalformedToolCall { tool_call_id } => {
            format!(
                "{}Model returned a tool call without a name (tool_call id={}). This provider's function-calling output is malformed — try a different model.",
                retry_prefix, tool_call_id
            )
        }
