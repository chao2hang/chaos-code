...
///
/// Chaos installs from its own GitHub repo. The upstream `channel` parameter
/// is kept for signature compatibility; the fork ships a single release
/// channel, so every channel resolves to the same one-liner.
    ...
    match installer {
        "npm" => {
            "请通过 npm 重新安装:\n  npm i -g chaos-code\n\n或使用 GitHub Release 一键安装:\n  "
                .to_string()
                + manual_install_cmd(channel).as_str()
        }
...
pub async fn get_installer() -> Option<&'static str> {
    ...
    match cfg.cli.installer.as_deref() {
        ...
        // process is actually npm-managed (env — handled above — or the
        // executable resolves inside node_modules).
        Some("npm") if path_resolves_to_npm_entry() => Some("npm"),
        ...
        // Exception: an npm-managed exe must keep updating through npm.
        Some("internal") | None if path_resolves_to_npm_entry() => Some("npm"),
...
async fn run_update_subcommand(
    ...
    match run_mode {
        UpdateRunMode::Blocking => {
            ...
            if !status.success() {
                anyhow::bail!("chaos update failed with {}", status);
            }
            Ok(None)
        }
    ...
    rename_result.map_err(|e| {
        anyhow::anyhow!(
            "cannot rename locked executable {}: {e}\n\
             Close all running chaos sessions and retry.",
            dest.display(),
        )
    })?;
    match tokio::fs::copy(src, dest).await {
...
fn warn_if_other_grok_processes_running() {
    ...
    if let Ok(output) = cmd.output() {
        ...
        if !other_pids.is_empty() {
            eprintln!(
                "  ⚠ Warning: {} other chaos process(es) detected.",
                other_pids.len()
            );
            ...
            eprintln!("    Consider closing other chaos sessions before updating.");
            eprintln!();
        }
    }
}
...
async fn refresh_deployment_config() {
    ...
    match xai_grok_shell::managed_config::sync().await {
        ...
        Err(e) if e.is_retryable() => {
            ...
            eprintln!("  Couldn't apply managed configuration. Run `chaos setup` to retry.");
        }
