...
- **SSH sessions** now show a one-time tip recommending `chaos wrap ssh <host>` for clipboard and terminal restore.
...
- **chaos wrap** now restores the terminal after SSH disconnects or other abrupt child exits.
- **Voice speech-to-text** now works with per-model API keys in config.toml without requiring `chaos login`.
