...
- **chaos login** now requests workspace project scopes so workspace listing works after consent.
...
- **`chaos plugin install`** now accepts a marketplace's registered name as a qualifier.
...
- **Fixed background task tracking** when using chaos -p --no-wait-for-background so tasks are properly reaped on exit.
