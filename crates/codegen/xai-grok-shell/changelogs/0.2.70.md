...
- **Added `chaos wrap`** to run any command with local clipboard support.
...
- **Fixed `chaos agent stdio` startup hangs** on Windows when used with persistent clients such as VS Code or chaos-desktop.
