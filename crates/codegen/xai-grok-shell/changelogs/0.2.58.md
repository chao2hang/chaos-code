...
- The legacy `agent` command is now kept in sync with `chaos` after running `chaos update`.
- Headless (`chaos -p`) runs now wait for background tasks and subagents to finish before exiting.
