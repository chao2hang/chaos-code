...
- **chaos inspect** now lists skills from [skills].paths and correctly labels bundled vs user skills.
...
- **Xcode / Foundation ACP clients** can now drive chaos agent stdio without silent parse drops on session/* calls.
...
- **chaos --minimal** now supports native drag-select on classic Windows conhost terminals.
