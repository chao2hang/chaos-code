...
/// (false). Overlay-free: the `GROK_CONFIG` / `GROK_CONFIG_PATH` overlay is
/// deliberately excluded, matching `ConfigLayers::env_overlay`'s contract.
/// This is an egress gate, so an overlay cannot re-enable a user's or a
/// deployment's "never fetch" decision. Callable before an `AgentConfig`
/// exists (startup prefetch runs pre-agent), so it re-reads the config
/// layers like `managed_config::is_fetch_enabled`.
///
...
/// managed-only pin also survives a corrupt user layer. The user
/// `config.toml` tier stays fail-open: it is a preference, not deployment
/// policy. With no policy tier present the knob resolves to the fork
/// default (off). Mirrors the `auto_permission_mode_enabled_from_disk`
/// soft-fail precedent.
