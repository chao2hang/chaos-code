    ...
    pub(crate) async fn set_model_gated(
        ...
        let model = match self.resolve_model_id(&args.model_id) {
            ...
            Err(_) => {
                ...
                // Re-read disk synchronously, wait for any in-flight catalog
                // fetch, then retry once. If the model is genuinely unknown
                // the retry fails with the same error.
        ...
        let result = match method.as_ref() {
            ...
            "x.ai/cloud/terminate" => {
                crate::extensions::auth_gate::require_xai_auth(
                    ...
                    crate::extensions::auth_gate::NO_XAI_SIGNIN,
                )?;
