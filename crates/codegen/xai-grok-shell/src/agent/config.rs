...
pub struct Requirements {
    ...
    /// Session search index pin. Default ON; turn off via a requirements.toml
    /// or MDM pin.
...
pub struct Config {
    ...
    /// `[adhd]` section: ADHD skill integration toggle. When `enabled`, the
    /// pager's `acp/mod.rs` injects ADHD-friendly rules into the system prompt
    /// via the `rules` metadata field. Toggled by the `/adhd` slash command,
    /// which persists `[adhd].enabled` in config.toml.
    #[serde(default)]
    pub adhd: AdhdConfig,
}
#[derive(Debug, Clone, Default)]
...
pub struct AgentSelectionConfig {
    ...
    /// Preset bundle name (`--preset` / `[agent] preset`): resolves a named
    /// bundle of toolset + persona + prompt. Higher priority than `name` /
    /// `definition`, so an explicit preset always wins.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl Config {
    ...
    /// Session search index. Default ON. Turn off with a `requirements.toml` or
    /// MDM pin, the `GROK_SESSION_SEARCH` env var, or the `[features]
    /// session_search` config key, in that order. Only a pin outranks the
    /// environment.
    ...
    /// Resolve the client profile declared for a model, if any.
    ///
    /// The resident session applies the live (per-`/client`) profile in
    /// `reconstruct_full_config` from the `SessionActor` header bags. This
    /// one-shot path (no resident session) consults the model's declared
    /// profile so identity headers still win per-key; `None` leaves the
    /// sampling config on its defaults.
    pub(crate) fn client_profile_for_model(
        &self,
        _model: &ModelEntry,
...
/// `[adhd]` section: ADHD skill integration. When `enabled`, ADHD-friendly
/// rules are injected into the system prompt. Toggled via the `/adhd` slash
/// command (pager-owned), which persists `[adhd].enabled` in config.toml.
#[derive(Clone, Debug, Default, Serialize, Deserialize)]
#[serde(default)]
pub struct AdhdConfig {
    /// Master switch for ADHD skill rule injection. Defaults to `false`.
    pub enabled: bool,
}
...
pub struct Features {
    ...
    /// Session search index. `None` = defer to env / default (`true`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub(crate) fn inject_url_derived_headers(
    ...
    // WorkBuddy clients must not forward `x-grok-*` / proxy identity headers,
    // so the cli-chat-proxy identity injection is skipped entirely.
    ...
    let _ = (alpha_test_key, base_url, is_workbuddy);
}
