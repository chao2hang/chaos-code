...
/// Guard via the feature registry (the fork's old `resolve_feedback`
/// resolver was replaced by `Feature::Feedback`).
#[test]
#[serial]
...
/// Chaos-fork: two-pass compaction stays opt-in (default off).
#[test]
#[serial]
...
fn a_status_line_the_parser_could_not_read_in_full_reaches_grok_inspect() {
    ...
    let warnings = |path: &str, kind: ConfigWarningKind| {
        cfg.config_warnings
            ...
            .filter(|w| {
                w.kind == kind
                    && matches!(&w.target, WarningTarget::ConfigKey { key: p } if p == path)
            })
            .count()
    };
    assert_eq!(
