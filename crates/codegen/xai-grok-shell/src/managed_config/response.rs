...
pub enum ManagedConfigError {
    ...
    #[error(
        "Your xAI team access was rejected: the credential expired or lacks access. Chaos does not sign in to xAI."
    )]
    TeamAuthRejected,
    ...
    SignatureRejected,
    #[error(
        "Can't save the configuration to ~/.chaos. Make sure the directory exists and is writable.\n  ({0})"
    )]
