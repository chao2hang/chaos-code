            ...
            Self::NotFound { name } => {
                write!(
                    ...
                    "Plugin \"{name}\" not found.\n\
                     Run `chaos plugin list` to see installed plugins."
                )
            }
            Self::NeedsConfirm {
                name,
                ...
                if registered.is_empty() {
                    write!(
                        ...
                        "Unknown marketplace \"{qualifier}\". No marketplaces are registered; \
                         add one with `chaos plugin marketplace add`."
                    )
                } else {
            ...
            Self::NameNotFound {
                ...
                write!(
                    ...
                    "No marketplace plugin named \"{name}\" in any registered marketplace.\n\
                     Install a local directory with `chaos plugin install ./{name}`, or add a \
                     source with `chaos plugin marketplace add`."
                )?;
            ...
            Self::NameAmbiguous { name, candidates } => {
                ...
                write!(
                    ...
                    "Multiple marketplaces provide a plugin named \"{name}\":\n{list}\n\
                     Pin one with `chaos plugin install {name}@<qualifier>`."
                )
            }
            Self::PartialScan {
                name,
                ...
                write!(
                    ...
                    "Couldn't scan every marketplace while resolving \"{name}\", so it can't be \
                     ...
                     Retry, or pin the source explicitly with `chaos plugin install {name}@<qualifier>`."
                )
            }
            Self::Sync {
                source_display,
...
mod tests {
    ...
    fn registered_source_label_uses_addressable_qualifier() {
        assert_eq!(
            ...
            "Official Marketplace (xai-org/plugin-marketplace)"
        );
        assert_eq!(
    ...
    fn candidate_label_includes_pin_hint() {
        assert_eq!(
            ...
            "Official Marketplace (pin: sentry@xai-org/plugin-marketplace)"
        );
        assert_eq!(
    ...
    fn unknown_qualifier_error_lists_registered_marketplaces() {
        let err = MarketplaceInstallError::UnknownQualifier {
            ...
            registered: vec![
                "Official Marketplace (xai-org/plugin-marketplace)".into(),
        ...
        assert!(
            msg.contains("  - Official Marketplace (xai-org/plugin-marketplace)"),
            "{msg}"
        );
    ...
    fn name_not_found_error_hints_local_dir_and_add_source() {
        ...
        let msg = err.to_string();
        assert!(msg.contains("chaos plugin install ./sentry"), "{msg}");
        assert!(msg.contains("chaos plugin marketplace add"), "{msg}");
    ...
    fn name_ambiguous_error_lists_candidates_and_pin_hint() {
        ...
        assert!(
            msg.contains("  - Official Marketplace (pin: sentry@xai-org/plugin-marketplace)"),
            "{msg}"
        );
        assert!(
            msg.contains("chaos plugin install sentry@<qualifier>"),
            "{msg}"
        );
    }
    ...
    fn plan_install_qualifier_unknown_lists_registered_labels() {
        let sources = [
            git_source("Official Marketplace", OFFICIAL_URL),
        ...
        match err {
            MarketplaceInstallError::UnknownQualifier {
                ...
                assert_eq!(
                    registered,
                    vec![
                        "Official Marketplace (xai-org/plugin-marketplace)".to_string(),
    ...
    fn plan_install_qualifier_not_found_when_scan_lacks_name() {
        let sources = [git_source("Official Marketplace", OFFICIAL_URL)];
        let err = plan_install(
            &sources,
            "sentry",
            Some("xai-org/plugin-marketplace"),
        ...
        match err {
            MarketplaceInstallError::QualifiedNameNotFound {
                ...
                assert_eq!(source_display, "Official Marketplace");
            }
    ...
    fn plan_install_bare_name_official_match_proceeds_despite_skip() {
        ...
        let plan = plan_install(&sources, "sentry", None, |source| {
            if source.name == "Official Marketplace" {
                Ok(vec![mp_entry("sentry")])
            } else {
                Err("sync failed".to_string())
            }
    ...
    fn resolve_qualified_source_name_with_matches_git_owner_repo() {
        ...
        assert_eq!(name, "Official Marketplace");
    }
