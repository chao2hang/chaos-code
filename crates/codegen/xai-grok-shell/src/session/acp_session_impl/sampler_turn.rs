...
pub(super) fn transient_retry_eligible(error: &xai_grok_sampler::SamplingErrorInfo) -> bool {
    ...
    match error.kind {
        ...
        // Chaos-fork: malformed tool calls are recovered inside the sampler's own
        // attempt loop (strip + feedback), never via a whole-turn transient retry
        ...
        if auth_mode == crate::auth::AuthMode::WebLogin {
            let msg = format!(
                "{detailed_message}\n\n\
                 ...
                 To fix: delete the stale `auth.json` in your config home, then configure your provider in config.toml.\n\n\
