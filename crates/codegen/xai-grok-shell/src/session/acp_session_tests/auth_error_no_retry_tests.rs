    ...
    local
        .run_until(async {
            ...
            assert!(
                ...
                "hint must name the stale credential, got: {msg}"
            );
            assert!(
                msg.contains("config.toml"),
                "hint must point at the provider configuration, got: {msg}"
            );
            assert!(
                !msg.contains("grok login") && !msg.contains("grok logout"),
                "hint must not name a sign-in Chaos does not offer, got: {msg}"
            );
            assert!(
