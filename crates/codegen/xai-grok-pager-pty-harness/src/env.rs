...
fn ensure_local_pager_binary(binary: &std::path::Path) -> Result<()> {
    ...
    let output = cmd
        .output()
        .with_context(|| format!("failed to spawn {cargo} to build chaos"))?;

    if !output.status.success() {
        bail!(
            "failed to build chaos (exit {:?})\nstdout:\n{}\nstderr:\n{}",
            output.status.code(),
...
/// 2. `CARGO_BIN_EXE_chaos` (set by `cargo test`)
/// 3. Build locally via `cargo build -p xai-grok-pager-bin` (the composition-
///    root package that owns the `chaos` binary)
pub fn pager_binary() -> Result<PathBuf> {
    ...
    if let Ok(path) = std::env::var("CARGO_BIN_EXE_chaos") {
        let p = PathBuf::from(path);
        if p.exists() {
            return Ok(p);
        }
