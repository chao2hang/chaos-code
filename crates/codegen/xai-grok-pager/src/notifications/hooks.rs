...
fn execute_hook(
    ...
    if let Some(sid) = session_id {
        ...
        // `Command` inherits the parent environment, so a Grok session running under another one would
        // otherwise leak that session's id into a hook the caller never gave an id for.
