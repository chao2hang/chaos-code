...
pub enum SessionEvent {
    ...
    /// Carries the goal's total elapsed time across all its turns, distinct from the per-turn "耗时" marker.
    GoalCompleted {
...
mod tests {
    ...
    fn compaction_failed_two_line_error_renders_headline_then_detail() {
        ...
        assert_eq!(
            plain(&out.lines[0]),
            "压缩失败：it'll retry on the next turn, or start a new session using /new."
        );
        assert_eq!(
            plain(&out.lines[1]),
            "API error (status 400 Bad Request): invalid_image: too big"
