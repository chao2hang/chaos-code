...
pub fn builtin_commands() -> Vec<Arc<dyn SlashCommand>> {
    vec![
        ...
        // Chaos: BYOK provider / preset management.
...
mod tests {
    ...
    fn model_suggest_args_returns_available_models() {
        ...
        assert!(
            items
                .iter()
                .any(|i| i.display.starts_with("Grok 4.5") && i.insert_text.trim() == "Grok 4.5")
        );
        assert!(
            items
                .iter()
                .any(|i| i.display == "Grok 4.3" && i.insert_text.trim() == "Grok 4.3")
        );
    }
    #[test]
