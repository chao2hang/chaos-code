...
//! Chained autocomplete: after picking a reasoning-supported model, the trailing space re-opens the dropdown into a `max|xhigh|high|medium|low` sub-menu.
...
mod tests {
    ...
    fn empty_query_returns_one_row_per_logical_model() {
        ...
        // (matched by display: fork match_text also carries the model id).
        let plain = items
            .iter()
    ...
    fn trailing_space_after_reasoning_model_enters_effort_phase() {
        ...
        // Items come out ordered max to low (strongest first) per EFFORT_LEVELS
