...
pub struct SuggestionRow {
    ...
    pub indices: Vec<u32>,
    /// Optional free-form tag (e.g. `new`, `beta`) rendered as `[tag]` after
    /// the command name in the dropdown. Filled from the shared command-tags
    /// map keyed by canonical name; `None` for untagged rows and all arg rows.
