...
fn render_list(buf: &mut Buffer, area: Rect, state: &mut ClientModalState, theme: &Theme) {
    ...
    Line::from(Span::styled(active, Style::default().fg(theme.accent_user)))
        ...
        .take(available.max(1))
    {
        ...
        let kind = if ClientModalState::is_builtin(profile) {
            "内置"
        } else {
            "自定义"
        };
        ...
        let line = Line::from(vec![
            ...
            Span::styled(
                format!(
                    "  [{}]  {} / {}  {}{}",
                    profile.id, profile.protocol, profile.auth_scheme, kind, default
                ),
...
fn render_form(buf: &mut Buffer, area: Rect, state: &ClientModalState, theme: &Theme) {
    ...
    let intro = if editing {
        ...
        "新增自定义客户端（不保存 API Key）"
    };
    Line::from(Span::styled(intro, Style::default().fg(theme.gray_bright)))
        .render(Rect::new(area.x, y, area.width, 1), buf);
    y += 2;
    ...
    Line::from(Span::styled(
        "认证为 none 时环境变量可留空；仅保存变量名，不保存密钥。",
        Style::default().fg(theme.gray_dim),
    ))
...
fn render_confirm(buf: &mut Buffer, area: Rect, state: &ClientModalState, id: &str, theme: &Theme) {
    ...
    for (offset, line) in lines.iter().enumerate() {
        ...
        .render(
            Rect::new(area.x, area.y + offset as u16 * 2, area.width, 1),
            buf,
        );
    }
    render_message(buf, area, state, theme);
...
fn render_message(buf: &mut Buffer, area: Rect, state: &ClientModalState, theme: &Theme) {
    ...
    Line::from(Span::styled(message.to_owned(), style))
        .render(Rect::new(area.x, y, area.width, 1), buf);
}
