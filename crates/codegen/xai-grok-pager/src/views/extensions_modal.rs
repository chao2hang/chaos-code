...
/// Build merged hook groups (same grouping as the renderer does).
...
pub fn next_visible_hook(
    ...
    for (gi, group) in groups.iter().enumerate() {
        if group.indices.contains(&current) {
    ...
    if let Some(gi) = current_group_idx {
        ...
        if !collapsed.contains(group.source_dir)
            && let Some(&next) = group.indices.iter().find(|&&i| i > current)
        {
            return Some(next);
        }
        // Move to next group's first hook.
        if gi + 1 < groups.len() {
            return groups[gi + 1].indices.first().copied();
        }
    } else {
        // current not found (filtered out): find first group with any index > current.
        for group in &groups {
            if let Some(&next) = group.indices.iter().find(|&&i| i > current) {
                return Some(next);
            }
        }
    }
...
pub fn prev_visible_hook(
    ...
    if let Some(gi) = current_group_idx {
        ...
        // If group is expanded, try to move within the group.
        if !collapsed.contains(group.source_dir)
            && let Some(&prev) = group.indices.iter().rev().find(|&&i| i < current)
        {
            return Some(prev);
        }
        // Move to previous group's representative.
        if gi > 0 {
            let prev_group = &groups[gi - 1];
            return if collapsed.contains(prev_group.source_dir) {
                prev_group.indices.first().copied()
            } else {
                prev_group.indices.last().copied()
            };
        }
    } else {
        ...
        for group in &groups {
            if let Some(&prev) = group.indices.iter().rev().find(|&&i| i < current) {
                best = Some(prev);
            }
        }
        return best;
...
pub fn extensions_action_keys(tab: ExtensionsTab) -> Vec<(char, &'static str)> {
    match tab {
        ...
        ExtensionsTab::Workflows => vec![('r', "重载")],
...
mod tests {
    ...
    fn derive_source_label_does_not_eat_neighbor_profile() {
        ...
        assert_eq!(label, format!("自定义: {neighbor}"));
        assert!(is_custom);
