...
示例：
  # 添加一个 stdio 服务器（-- 之后的内容即为服务器命令）
  chaos mcp add xcode -- xcrun mcpbridge

  # 添加一个带环境变量的 stdio 服务器
  ...
  # 添加到项目配置（./.grok/config.toml）而不是 ~/.chaos/config.toml
  chaos mcp add --scope project github -- npx -y @modelcontextprotocol/server-github";
...
pub enum McpTransport {
    /// 启动本地进程，并通过 stdin/stdout 通信
    Stdio,
    /// 通过可流式传输的 HTTP 连接远程服务器
    Http,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
pub enum McpScope {
    /// `~/.chaos/config.toml`，在你的所有项目中可用
    User,
    /// `./.grok/config.toml`，与在此目录中工作的所有人共享
    Project,
}
...
pub struct AddArgs {
    ...
    /// 传给服务器命令的参数。
    /// 请放在 `--` 之后，这样诸如 `-y` 的标志会传给服务器而不是 chaos。
    #[arg(value_name = "ARGS")]
    args: Vec<String>,

    /// 传输类型。默认为 stdio；当位置参数是 http(s):// URL 时默认为 http。
    #[arg(short = 't', long, value_enum)]
    transport: Option<McpTransport>,

    /// 写入哪个配置：user（~/.chaos/config.toml）或 project（./.grok/config.toml）
...
fn run_list(json: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("No MCP servers configured. Run `chaos mcp add --help` to get started.");
    } else {
    ...
    match transport {
        McpTransport::Stdio => {
            let Some(command) = source else {
                bail!(
                    "A command is required for stdio servers. Usage: chaos mcp add <name> -- <command> [args...]"
                );
            };
        ...
        McpTransport::Http | McpTransport::Sse => {
            ...
            let Some(url) = source else {
                bail!(
                    "A URL is required for {label} servers. Usage: chaos mcp add --transport {label} <name> <url>"
                );
            };
...
async fn run_set_enabled(name: &str, enabled: bool) -> Result<()> {
    ...
    if !mcp_server_is_known(name, &cwd) {
        ...
        if !available.is_empty() {
            ...
            eprintln!("No MCP servers configured. Run `chaos mcp add --help` to get started.");
        }
        std::process::exit(1);
    }
...
async fn run_remove(name: &str, requested_scope: Option<McpScope>) -> Result<()> {
    ...
    let (scope, path) = match select_remove_site(user_defined, find_project_site(), requested_scope)
    {
        ...
        Err(RemoveError::Ambiguous { project_path }) => {
            ...
            eprintln!("Specify which one to remove, e.g.: chaos mcp remove {name} --scope project");
            std::process::exit(1);
        }
    };
