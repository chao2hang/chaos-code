...
impl AgentView {
    ...
    fn question_shortcut_hints(
        ...
        match qv.focus {
            ...
            QuestionFocus::Navigation if qv.is_prompt_blocked() => {
                vec![HintItem::new(key!(Tab), "下一个答案"), esc]
            }
    ...
    fn permission_shortcut_hints(
        ...
        match perm.focus {
            PermissionFocus::FollowupInput => {
                let mut hints = vec![HintItem::new(key!(Enter), "发送")];
                hints.extend(ctrl_f_hint);
                hints.push(self.card_esc_hint());
                hints
            }
            PermissionFocus::PatternEdit => {
                let mut hints = vec![HintItem::new(key!(Enter), "保存")];
                hints.extend(ctrl_f_hint);
                hints.push(self.card_esc_hint());
                hints
            }
    ...
    fn shortcuts_bar_content(
        ...
        match self.key_owner() {
            ...
            KeyOwner::Card(BlockingCard::McpElicitation) => ShortcutsBarContent::Surface(vec![
                HintItem::new(key!(Enter), "接受 / 切换"),
                HintItem::new(key!('d'), "拒绝"),
                HintItem::new(key!('c', CONTROL), "取消"),
                HintItem::new(key!(Tab), "下一字段"),
                self.card_esc_hint(),
            ]),
            KeyOwner::Pane => {
        ...
        let activity_label: Option<String> = if is_running {
            self.subagent_views.get(child_sid).and_then(|cv| {
                cv.resolve_turn_activity()
                    ...
                    .or_else(|| cv.session.state.is_busy().then(|| "等待中".to_string()))
            })
        } else {
            None
        };
    ...
    pub fn draw(
        ...
        if self.should_show_plan_chip(&appearance) {
            ...
            status.push("计划", Line::from(Span::styled("计划", plan_style)));
        }
        ...
        if let Some(ref viewer) = self.block_viewer {
            ...
            let mut hints = self.normal_pane_hints(registry, false);
        ...
        if let Some(ref mut viewer) = self.line_viewer {
            ...
            let viewer_hints = if in_plan_approval && on_comment {
                ...
                let mut h = vec![HintItem::new(key!('c'), "批注")];
                if approval_has_comments {
                    h.push(HintItem::new(key!('s'), "发送"));
                } else {
                    h.push(HintItem::new(key!('a'), "批准"));
