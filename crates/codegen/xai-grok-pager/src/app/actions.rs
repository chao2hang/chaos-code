...
pub enum Action {
    ...
    /// Toggle incomplete `end_turn` auto-retry. SHELL-owned; persisted to `[session].auto_retry_incomplete_end_turn`. Applies to new sessions.
    ...
    ShowContextInfo,
    /// Dynamically set the current session's context window (tokens). When shrinking, the shell may compact history to fit.
    SetContextWindow {
        tokens: u64,
    ...
    /// The `[+ New Agent]` button then reads `[+ 新建 Worktree]`. Bound to Ctrl+W.
    ...
    /// Focus the header's `[+ 新建会话]` button.
