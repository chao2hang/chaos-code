...
fn doctor_fix_target(agent: &AgentView) -> DoctorFixTarget {
    // Epoch tracking is optional hardening; Chaos keeps the target keyed by
    // agent/session/cwd. Zero is a stable sentinel until AgentView gains a
    // binding epoch counter.
    DoctorFixTarget {
