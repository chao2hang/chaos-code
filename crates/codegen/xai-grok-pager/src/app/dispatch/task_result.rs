...
pub(super) fn dispatch_task_result(result: TaskResult, app: &mut AppView) -> Vec<Effect> {
    match result {
        ...
        TaskResult::CatPawQrStarted { provider, qr } => {
            use crate::views::modal::ActiveModal;
            use crate::views::provider_modal::CatPawLoginPhase;
            for agent in app.agents.values_mut() {
                if let Some(ActiveModal::ProviderModal { state }) = &mut agent.active_modal
                    && matches!(
                        state.mode,
                        crate::views::provider_modal::ProviderModalMode::CatPawLogin(ref p)
                            if *p == provider
                    )
                {
            ...
            // Schedule the first poll in 3s (the effect sleeps first).
            vec![Effect::CatPawPollQrLogin {
                provider,
                code: qr.code,
            }]
        }
        TaskResult::CatPawQrPolled { provider, poll } => {
            use crate::views::modal::ActiveModal;
            use crate::views::provider_modal::CatPawLoginPhase;
            use xai_grok_shell::catpaw::QrStatus;
            let mut next_poll: Option<Effect> = None;
            ...
            for agent in app.agents.values_mut() {
                ...
                {
                    let current_code = match &state.catpaw_login {
                        Some(CatPawLoginPhase::ShowQr { code, .. }) => Some(code.clone()),
                        _ => None,
                    };
                    match poll.status {
                        QrStatus::Ok => {
                            let tokens = xai_grok_shell::catpaw::TokenSet {
                                access_token: poll.access_token.clone().unwrap_or_default(),
                                refresh_token: poll.refresh_token.clone().unwrap_or_default(),
                                expires: poll.expires.unwrap_or_default(),
                                refresh_expires: poll.refresh_expires.unwrap_or_default(),
                                mis_id: poll.mis_id.clone(),
                                user_info: None,
                            };
                            match xai_grok_shell::catpaw::insert_account(&provider, None, &tokens) {
                                ...
                                Err(e) => {
                                    state.catpaw_login =
                                        Some(CatPawLoginPhase::Error(e.to_string()));
                                }
                            }
                        }
                        QrStatus::Scanned => {
                            state.catpaw_login = Some(CatPawLoginPhase::Scanned);
                            next_poll = current_code.map(|code| Effect::CatPawPollQrLogin {
                        ...
                        QrStatus::Expired => {
                            state.catpaw_login = Some(CatPawLoginPhase::Expired);
                        }
                    }
                    break;
                }
            ...
            if close_modal {
                for agent in app.agents.values_mut() {
                    if let Some(ActiveModal::ProviderModal { .. }) = &agent.active_modal {
                        agent.active_modal = None;
                        break;
                    }
                }
            }
            next_poll.into_iter().collect()
        }
        TaskResult::CatPawLoginFailed { provider, error } => {
            use crate::views::modal::ActiveModal;
            use crate::views::provider_modal::CatPawLoginPhase;
            for agent in app.agents.values_mut() {
                if let Some(ActiveModal::ProviderModal { state }) = &mut agent.active_modal
                ...
                {
                    state.catpaw_login = Some(CatPawLoginPhase::Error(error));
                    break;
                }
            }
            vec![]
