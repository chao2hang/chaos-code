...
type ServerMessage = { type: string; session_id?: string; request_id?: string; question_id?: string; tool?: string; summary?: string; prompt?: string; text?: string; answer?: string; messages?: Message[]; protocol_version?: number }
...
function App() {
  ...
  const connect = useCallback(() => {
    ...
    ws.onmessage = (event) => {
      ...
      if (message.type === 'question_requested' && message.question_id) { setQuestion({ questionId: message.question_id, prompt: message.prompt ?? '' }); setBusy(false); setStatus('等待回答') }
  ...
  function answerQuestion(answer: string) { if (!question || !answer.trim()) return; send({ type: 'respond_question', client_msg_id: crypto.randomUUID(), question_id: question.questionId, answer }); setQuestion(undefined) }

  return <main className="shell"><header><strong>Chaos</strong><span>{status} · Web / Desktop</span></header><section className="timeline" aria-label="会话时间线">{messages.length === 0 && <p className="empty">创建会话后，在下方输入 Prompt。</p>}{messages.map((message, index) => <article className={message.role} key={index}><small>{message.role === 'user' ? '你' : 'Chaos'}</small><p>{message.text || '正在生成…'}</p></article>)}{approval && <article className="approval" aria-label="工具审批"><strong>需要审批：{approval.tool}</strong><p>{approval.summary}</p><div><button type="button" onClick={() => resolveApproval(true)}>允许</button><button type="button" onClick={() => resolveApproval(false)}>拒绝</button></div></article>}{question && <article className="approval" aria-label="问题"><strong>需要回答</strong><p>{question.prompt}</p><div><button type="button" onClick={() => answerQuestion('是')}>是</button><button type="button" onClick={() => answerQuestion('否')}>否</button></div></article>}</section><form onSubmit={(event) => { event.preventDefault(); submit() }}><textarea aria-label="Prompt" value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder="输入 Prompt" /><button type="submit" disabled={busy || !prompt.trim()}>发送</button>{busy && <button type="button" onClick={cancel}>停止</button>}</form></main>
}
