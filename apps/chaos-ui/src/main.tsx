...
function App() {
  ...
  const [layout, setLayout] = useState<LayoutState>(() => typeof window === 'undefined' ? defaultLayoutState : loadLayoutState(window.localStorage))
  const socket = useRef<WebSocket | null>(null)
  const sessionStateRef = useRef(session)
  const reconnectTimer = useRef<number | undefined>(undefined)

  useEffect(() => { sessionStateRef.current = session }, [session])
  useEffect(() => { if (typeof window !== 'undefined') saveLayoutState(window.localStorage, layout) }, [layout])
  useEffect(() => {
