...
function App() {
  ...
  return <main className="shell" data-testid="app-shell"><header><strong>Chaos</strong><span><span role="status" aria-live="polite" data-testid="session-status">{session.status}</span> · Web / Desktop <button type="button" onClick={togglePanel}>{layout.panelOpen ? '隐藏面板' : '显示面板'}</button> <button type="button" onClick={cycleTheme}>主题：{layout.theme}</button></span><label>面板宽度 <input aria-label="面板宽度" type="range" min="160" max="480" step="10" value={layout.sidebarWidth} onChange={(event) => setLayout((current) => ({ ...current, sidebarWidth: Number(event.target.value) }))} /></label><label>输入区高度 <input aria-label="输入区高度" type="range" min="88" max="480" step="8" value={layout.composerHeight} onChange={(event) => setLayout((current) => ({ ...current, composerHeight: Number(event.target.value) }))} /></label></header><aside hidden={!layout.panelOpen} className="workspaces" aria-label="工作区" data-testid="workspace-list"><button type="button" onClick={createWorkspace}>+ 新工作区</button>{session.workspaces.filter((workspace) => !workspace.archived).map((workspace) => <div className="workspace-item" key={workspace.id}><button type="button" data-testid={`workspace-${workspace.id}`} className={workspace.id === session.activeWorkspaceId ? 'active' : ''} onClick={() => switchWorkspace(workspace.id)}>{workspace.name}<small>{workspace.id === session.activeWorkspaceId ? '当前' : '切换'}</small></button><button type="button" className="workspace-archive" aria-label={`归档工作区 ${workspace.name}`} title="归档此工作区" onClick={() => archiveWorkspace(workspace.id)}>归档</button></div>)}</aside><section className="timeline" aria-label="会话时间线" data-testid="session-timeline">{session.messages.length === 0 && <p className="empty">创建会话后，在下方输入 Prompt。</p>}{session.messages.map((message, index) => <article className={message.role} key={index}><small>{message.role === 'user' ? '你' : 'Chaos'}</small><div className="markdown-body"><ReactMarkdown remarkPlugins={[remarkGfm]} components={{ a: ({ href, children }) => {
