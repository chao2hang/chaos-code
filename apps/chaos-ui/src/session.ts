...
export const initialSessionState: SessionState = { messages: [], busy: false, status: '连接中' }

export function applyServerMessage(state: SessionState, message: ServerMessage): SessionState {
  if (message.type === 'session_created' && message.session_id) return { ...state, sessionId: message.session_id, status: '会话已创建' }
