...
- M2 workspace boundary canonicalizes a configured root and supports bounded
  ...
  state persists its workspace binding, and Resume/Snapshot reject cross-workspace
  requests; React now renders real list/switch/archive events and includes the
  active workspace in resume. Full multi-workspace UI/layout isolation remains open.
