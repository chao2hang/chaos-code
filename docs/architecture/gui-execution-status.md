...
- M2 workspace boundary canonicalizes a configured root and supports bounded
  ...
  snapshot. Empty workspaces receive a session on first switch, and create/archive
  operations return a refreshed workspace registry. Reducer tests cover
  workspace-local session selection and empty-workspace handling. Full tabs/layout
  isolation and Desktop flow remain open.
