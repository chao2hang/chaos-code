...
- M3: settings/workflow views after settings schema and workflow/subagent event/state contracts are selected. A local accessibility slice adds theme-aware visible keyboard focus, semantic separate switch/archive buttons, and covers Tab focus plus pointer-free Enter submission in desktop/mobile Playwright; full localization, long-text layout and broader assistive-technology acceptance remain open. The current demo `/ask` and `/approve-tool` prompt hooks are not production workflow execution.
