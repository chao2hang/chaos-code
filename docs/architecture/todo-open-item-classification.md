...
- M2: Engine/WebSocket already exposes bounded single-host-root list/read/search, but no rendered file-browser UI yet. The real WebSocket flow now verifies sorted root entries, an empty directory response, `.chaos-staging` exclusion, directory-read `read_failed`, and out-of-root read rejection (`crates/codegen/xai-grok-web/tests/workspace_flow.rs`). `files_listed` still sends filename strings only, so the client cannot identify directories safely without guessing from filename extensions; root-relative navigation, loading/error/empty states and search-result integration need an agreed view contract plus browser E2E. Multi-root additionally requires an owner-approved host-owned root ID/map and root provisioning/trust policy. The complete interactive terminal route remains open.
