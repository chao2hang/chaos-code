...
- M2: Engine/WebSocket already exposes bounded single-host-root list/read/search, but no rendered file-browser UI yet. The real WebSocket flow now verifies sorted root entries, an empty directory response, root/nested `.chaos-staging` exclusion from parent listings, `path_escape` when directly reading/listing or proposing an approved write under the internal staging directory, directory-read `read_failed`, an actual search result, and out-of-root read rejection (`crates/codegen/xai-grok-web/tests/workspace_flow.rs`, `workspace_file_access_rejects_attachment_staging_paths` in `crates/codegen/chaos-engine/src/lib.rs`). The Engine test also asserts the attempted staging write does not create a file. The Engine also rejects direct `.chaos-staging` list/read/write paths as `path_escape` while retaining internal attachment finalization; `WorkspaceAdapter::search` prunes `.chaos-staging` before traversal so staged attachment contents are not searchable. `files_listed` still sends filename strings only, so the client cannot identify directories safely without guessing from filename extensions; root-relative navigation, loading/error/empty states and search-result integration need an agreed view contract plus browser E2E. `WorkspaceAdapter::write` also rejects dangling symlinks to paths outside the configured root through the real Engine approval/write path (`approved_write_rejects_dangling_symlink_escape` in `crates/codegen/chaos-engine/src/lib.rs`); this prevents `std::fs::write` from following a broken outside symlink to create a file externally. Multi-root additionally requires an owner-approved host-owned root ID/map and root provisioning/trust policy. The complete interactive terminal route remains open.
