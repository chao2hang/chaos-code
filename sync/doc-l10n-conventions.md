...
> **`--glob` 是仓库根相对路径，不能只写章节文件名。** `expand()` 走的是
> `Path().glob()`，`--glob '10-hooks.md'` 一个文件也匹配不到。早先脚本对
> 「匹配到 0 个文件」是静默返回空列表，于是所有计数都成了空转的 0 —— 一道
> 假通过。现已改为**硬报错**（`glob ... matched no file(s)`），但仍要按上面
> 的 `$G/<本章>` 形式写。教训是：任何「0」都要能被证伪，拿一个已知非零的
> 文件跑同一命令做**正对照**，确认 glob 真的解析到了文件。
