...
```sh
curl -fsSL https://raw.githubusercontent.com/chao2hang/chaos-code/main/scripts/install.sh | bash -s -- --version 0.2.138
...
```sh
./scripts/install.sh
./scripts/install.sh --version 0.2.138
```
...
```powershell
& ([scriptblock]::Create((irm https://raw.githubusercontent.com/chao2hang/chaos-code/main/scripts/install.ps1))) -Version 0.2.138
```
...
powershell -ExecutionPolicy Bypass -File "$env:TEMP\install-chaos.ps1" -Version 0.2.138
```
...
```bat
"%TEMP%\install-chaos.bat" --version 0.2.138
...
```bat
scripts\install.bat
scripts\install.bat --version 0.2.138 --force
```
