...
rem Signature verification is mandatory unless CHAOS_SKIP_SIGNATURE=1.
rem This fallback uses Python + cryptography because certutil cannot verify
rem ed25519; missing prerequisites or a missing sidecar fail closed.
if not "%CHAOS_SKIP_SIGNATURE%"=="1" (
  where python >nul 2>&1
  if errorlevel 1 (
    echo error: python is required for signature verification.
  ...
  if errorlevel 1 (
    echo error: Python package cryptography is required for signature verification.
    echo   To bypass ^(NOT recommended^), set CHAOS_SKIP_SIGNATURE=1.
    exit /b 1
  )
  ...
  if not exist "!SIG_FILE!" (
    echo error: required signature sidecar unavailable for %ASSET%
    exit /b 1
  )
  if exist "!SIG_FILE!" (
